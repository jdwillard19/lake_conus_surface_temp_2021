...
for k in k_arr:
	...
	#add noise optional
	augment[:,:,:-1] = augment[:,:,:-1] + (.005**.5)*np.random.randn(augment.shape[0],augment.shape[1],augment.shape[2]-1)
	augment[:,:,-1] = augment[:,:,-1] + (.05**.5)*np.random.randn(augment.shape[0],augment.shape[1])

	data = np.concatenate((data,augment), axis=0)
	np.save("../evaluate/ealstm_trn_data_oversamp_normal3_k"+str(int(k))+".npy",data)
