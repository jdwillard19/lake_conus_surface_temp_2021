...
trn_data = torch.from_numpy(np.load("ealstm_trn_data_oversamp7_k"+str(k)+".npy"))
...
if not train:
    load_path = "../../models/EALSTM_err_est_"+str(k)+"_newparam_oversamp7"
    if use_gpu:
    ...
    for epoch in range(n_eps):
        ...
        if avg_loss < min_train_rmse:
            ...
            print("model saved")
            save_path = "../../models/EALSTM_err_est_"+str(k)+"_newparam_oversamp7"
...
final_output_df.to_feather("../../results/err_est_outputs_072621_EALSTM_fold"+str(k)+"_oversamp7.feather")
save_path = "../../models/EALSTM_fold"+str(k)+"_newparam_oversamp6"
saveModel(lstm_net.state_dict(), optimizer.state_dict(), save_path)
