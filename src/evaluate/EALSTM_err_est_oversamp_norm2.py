...
targ_ep = hp[(hp['fold'])==k]['n_ep'].values[0]
targ_rmse = hp[(hp['fold'])==k]['trn_rmse'].values[0]
...
trn_data = torch.from_numpy(np.load("ealstm_trn_data_oversamp_normal2_k"+str(k)+".npy"))
...
if not train:
    load_path = "../../models/EALSTM_err_est_"+str(k)+"_newparam_oversamp_norm2"
    if use_gpu:
    ...
    for epoch in range(n_eps):
        ...
        if avg_loss < min_train_rmse:
            ...
            print("model saved")
            save_path = "../../models/EALSTM_err_est_"+str(k)+"_newparam_oversamp_norm2"
...
final_output_df.to_feather("../../results/err_est_outputs_072621_EALSTM_fold"+str(k)+"_oversamp_norm2.feather")
