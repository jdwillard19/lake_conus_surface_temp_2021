...
for name in folds_arr:
    ...
    header = "#!/bin/bash -l\n#SBATCH --time=23:59:00\n#SBATCH --ntasks=8\n#SBATCH --mem=20g\n#SBATCH --mail-type=ALL\n#SBATCH --mail-user=willa099@umn.edu\n#SBATCH --output=EALSTM_%s_group.out\n#SBATCH --error=EALSTM_%s_group.err\n#SBATCH --gres=gpu:v100:1\n#SBATCH -p v100"%(l,l)
    script = "source /home/kumarv/willa099/takeme_evaluate.sh\n" #cd to directory with training script
    script2 = "python EALSTM_err_est_grouploss_full.py %s"%(l)
    # script3 = "python singleModel_customSparse.py %s"%(l)
    all= "\n".join([header,script,script2])
    sbatch = "\n".join(["sbatch job_%s_foldEAgroupfull.sh"%(l),sbatch])
    with open('../../hpc/job_{}_foldEAgroupfull.sh'.format(l), 'w') as output:
        output.write(all)

compile_job_path= '../../hpc/sbatch_script_err_est_eagroupfull.sh'
