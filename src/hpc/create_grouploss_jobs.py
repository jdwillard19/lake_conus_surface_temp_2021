...
for name in folds_arr:
    ...
    script2 = "python EALSTM_err_est_grouploss.py %s"%(l)
    # script3 = "python singleModel_customSparse.py %s"%(l)
    all= "\n".join([header,script,script2])
    sbatch = "\n".join(["sbatch job_%s_foldEAgroup.sh"%(l),sbatch])
    with open('../../hpc/job_{}_foldEAgroup.sh'.format(l), 'w') as output:
        output.write(all)

compile_job_path= '../../hpc/sbatch_script_err_est_eagroup.sh'
